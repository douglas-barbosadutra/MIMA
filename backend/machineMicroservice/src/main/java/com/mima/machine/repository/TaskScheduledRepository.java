...
public interface TaskScheduledRepository extends JpaRepository<TaskScheduled, Long> {
	//@Query(value = "select distinct task_scheduled from TaskScheduled task_scheduled left join fetch task_scheduled.fathers", countQuery = "select count(distinct task_scheduled) from TaskScheduled task_scheduled")
	...
	public int insertScheduledRelations(@Param("father_id") Long father_id,
			@Param("task_scheduled_id") Long task_scheduled_id);*/
