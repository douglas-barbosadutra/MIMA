...
public class InputResource {
	...
	private static final String ENTITY_NAME = "machineMicroserviceInput";
	 ...
	 *         inputDTO, or with status 400 (Bad Request) if the input has already
	 *         an ID
	 ...
	 *         inputDTO, or with status 400 (Bad Request) if the inputDTO is not
	 *         valid, or with status 500 (Internal Server Error) if the inputDTO
	 *         couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/inputs")
	public ResponseEntity<InputDTO> updateInput(@RequestBody InputDTO inputDTO) throws URISyntaxException {
		...
		InputDTO result = inputService.save(inputDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, inputDTO.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of inputs in
	 *         body
	 */
	@GetMapping("/inputs")
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the inputDTO,
